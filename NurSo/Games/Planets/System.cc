...
public:
 ...
 {
   loc-=5;
   if(loc<0||Galaxy[loc]->who()!=5&&Galaxy[loc]->who()!=2&&loc!=dest){loc+=5;}
 }
 void shDo(int& loc,int dest)
 {
   loc+=5;
   if(loc>24||Galaxy[loc]->who()!=5&&Galaxy[loc]->who()!=2&&loc!=dest){loc-=5;}
 }
 void shLe(int& loc,int dest)
 {
   loc-=1;
   if(loc%5==4||loc<0||Galaxy[loc]->who()!=5&&Galaxy[loc]->who()!=2&&loc!=dest){loc+=1;}
 }
 void shRi(int& loc,int dest)
 {
   loc+=1;
   if(loc%5==0||Galaxy[loc]->who()!=5&&Galaxy[loc]->who()!=2&&loc!=dest){loc-=1;}
 }
 int bestMove(int loc,int dest)
 {
   if(loc==dest){return 4;}
   int saveloc=loc;
   int bestmove=0;
   int minabstand=89;
   for(int n=0;n<4;++n)
   ...
   if(minabstand==0&&Galaxy[dest]->who()==3||Galaxy[dest]->who()==1){return 5;}//"Landeanflug einleiten"
   return bestmove;
 }
 bool invade(int team,int where)
 {
  if(Galaxy[Zeiger]->Team==0&&(Galaxy[Zeiger]->who()!=4))
   {
 ...
 void alloc(int team,int where)
 {
   Galaxy[where]->Team=team;
   Galaxy[where]->_Hpr=1;
   Galaxy[where]->up();
 }
 void shipMove(int team,int loc,int dest)
 {
   ...
   switch(bestMove(loc,dest))
   {
     ...
     case 4:move=false;
     case 5:alloc(team,dest);break;
   }
   if(Galaxy[loc]->who()==2&&move)
   {
     Space* space=new Space();
